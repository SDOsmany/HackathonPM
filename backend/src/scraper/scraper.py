...
try:
    ...
    for p_tag in all_target_paragraphs:
        ...
        if is_main_info_paragraph:
            ...
            project_title = (
                title_strong_tag_in_a.get_text().strip()
            ...
            for strong_tag_in_p_list in all_strong_tags_in_p:
                ...
                if (
                    not strong_tag_in_p_list.find_parent("a") == anchor_tag
                    and not strong_tag_in_p_list == title_strong_tag_in_a
                ):
            ...
            next_sibling = (
                p_tag.find_next_sibling()
            )  # Get the next element in the HTML tree

            # Check if the next sibling is a p tag with the target style
            if (
                isinstance(next_sibling, Tag)
                and next_sibling.name == "p"
                and "style" in next_sibling.attrs
                and next_sibling["style"] == target_paragraph_style
            ):
    ...
    if MODE == "JSON":
        ...
        raise ValueError("Incorrect Mode: Must be 'JSON' or 'TXT'")
